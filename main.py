...
def game():
  ...
  while game_continue:
    ...
    print(f"Compare A: {format(celeb_a)}")
    print(vs)
    print(f"Against B: {format(celeb_b)}")
    guess = input("Who's followers are more? Type 'A' or 'B':").lower
    a_followers = celeb_a["follower_count"]
    b_followers = celeb_b["follower_count"]

    if check(guess, a_followers, b_followers):
      score += 1
      clear()
      print(logo)
